...
const template = `
<div class="ml-{{__called__}} mb-4 py-2 px-6 bg-white rounded-md shadow-xl ring-1 ring-slate-900/5 divide-y divide-slate-100 my-auto xl:mt-18 dark:bg-slate-800 dark:divide-slate-200/5 dark:highlight-white/10">
...
function makeComments(commentsList, called = 0) {
  ...
  for (let i = 0; i < commentsList.length; i++) {
    ...
    comment = comment.replace("{{__called__}}", 4 * called);
    ...
    if (commentsList[i].comments.length > 0) {
      result.push(makeComments(commentsList[i].comments, called + 1));
    }
  }
